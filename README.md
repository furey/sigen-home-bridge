...
- **Per-device breakdown**: Device Breakdown page lists multiple inverters behind plant totals (model, status, solar, active power, temperature, charge, health, and every PV string, each nameable in Settings) plus every active Smart Port load (live draw and lifetime energy, also nameable, with an optional combined readout on the dashboard, trends, and fullscreen views), reachable via optional dashboard button.
